...
const SIDEBAR_COLLAPSED_KEY = 'scm_workspace_sidebar_collapsed'
...
function IconPurchaseOrders() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.75} aria-hidden>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M7.5 3.75h7.5M7.5 3.75A1.875 1.875 0 005.625 5.625v14.625c0 1.035.84 1.875 1.875 1.875h9c1.035 0 1.875-.84 1.875-1.875V5.625A1.875 1.875 0 0016.5 3.75m-9 4.5h9m-9 3.75h9m-9 3.75h6"
      />
    </svg>
  )
}
...
function IconVendorPoPanel() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.75} aria-hidden>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M8.25 6.75h12M8.25 12h12m-12 4.5h6M3.75 4.5v4.5m0 4.5V18"
      />
    </svg>
  )
}

export function ScmDashboardLayout() {
  ...
  const vendorsDirActive = pathname.startsWith('/scm/vendors')
  const overviewActive =
    (pathname === '/scm' || pathname.startsWith('/scm/q/')) && !vendorsDirActive
  const purchaseOrdersActive = pathname.startsWith('/scm/purchase-orders')

  const initials = user?.displayName ? userInitials(user.displayName) : '?'
  const switchLabel = mode === 'azure' ? 'Switch account' : 'Switch user'

  return (
    <div className="sales-dash scm-dash">
      <aside
        className={`sales-dash__sidebar${collapsed ? ' sales-dash__sidebar--collapsed' : ''}`}
        aria-label="SCM workspace"
      >
        ...
        <nav
          ...
          aria-labelledby="scm-dash-workspace-label"
        >
          <Link
            to="/scm"
          ...
          >
            ...
            <span className="sales-dash__nav-text">Workspace</span>
          </Link>
          <Link
            to="/scm/purchase-orders"
            className={linkClass(purchaseOrdersActive)}
            aria-current={purchaseOrdersActive ? 'page' : undefined}
            title="Purchase orders"
            aria-label={collapsed ? 'Purchase orders' : undefined}
          >
            <span className="sales-dash__nav-icon">
              <IconPurchaseOrders />
            </span>
            <span className="sales-dash__nav-text">Purchase orders</span>
          </Link>
          <NavLink
            to="/scm/vendors"
          ...
          <NavLink
            ...
            aria-label={collapsed ? 'Vendors and PO' : undefined}
          >
            <span className="sales-dash__nav-icon">
              <IconVendorPoPanel />
            </span>
            <span className="sales-dash__nav-text">Vendors &amp; PO</span>
          </NavLink>
        </nav>
